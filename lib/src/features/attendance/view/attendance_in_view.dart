...
class AttendanceInView extends StatefulWidget {
  const AttendanceInView({super.key, required this.item});
  final SelectOutlet item;

  @override
  State<AttendanceInView> createState() => _AttendanceInViewState();
}

class _AttendanceInViewState extends State<AttendanceInView> {
  late final SelectOutlet item;
  ...
  Future<void> _getImage({required ImageSource source}) async {
    try {
      pickedFile = await _picker.pickImage(
          ...
          imageQuality: 85);
      if (pickedFile != null) {
        // Navigator.push(context, MaterialPageRoute(builder: (_) => CropGambar(pickedFile)))
        //     .then((value) async {
        //   if (value != null) {
        ...
        //     }
        //     _onButtonConfirmPress(file.path);
        //   }
        // });
  ...
  Future<void> _goToAttendanceDone() async {
    if (imagePath == null) return;
    bool proses_image = await _capturePng();
    _goToView(AttendanceDoneRoute(
        item: item,
        path: imagePath!,
        lang: lat.toString(),
        long: lon.toString()));
  ...
  void initState() {
    super.initState();
    item = widget.item;
    latStore = double.tryParse(widget.item.long!) ?? -7.3559213;
    lonStore = double.tryParse(widget.item.lang!) ?? 108.187251;
    _timer();
    setState(() {
      radius = widget.item.radius;
    });
  }
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: Constants.kPaddingM),
                XButton(
                  onPressed: _goToAttendanceDone,
                  backgroundColor:
                      pickedFile != null ? Colors.green : Colors.green.shade100,
                  child: const Text('Lakukan Absensi Masuk'),
                ),
                const SizedBox(height: Constants.kPaddingS),
                const AutoSizeText(
